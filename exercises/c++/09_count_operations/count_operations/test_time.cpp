...
void set_timed(const std::size_t n, I first, I last) {
  ...
  //typename is necessary when the type is nested like this; 
...
void vector_timed(const std::size_t n, I first, I last) {
  ...
  //the function unique() gets rid of the duplicates, doesn't change the length of the vector, and returns the iterator to the first "empty" element
  //example: 1 3 2 1 3 5 ---> 1 1 2 3 3 5 ---> 1 2 3 5 * * * 
  t.stop();
  //I also add the following line, because otherwise the compiler doesn't execute the line 27 if "it" is never used
...
int main() {
  ...
  for (std::size_t n = 16; n < (1 << 25); n <<= 1) { 
    ...
    for (std::size_t i = 0; i < n; ++i) {
      ...
      //in this way all the numbers between 0 and 1023 will stay the same; all the numbers above will be brought down to a number between 0 and 1023
      //ex: 1024 --> 0 ; 1025 --> 1 ecc.
    }
    std::cout << std::setw(15) << n << "\t";
    //set_timed(n,v.begin(),v.end()); //try to uncomment this and comment the following line to see what is better (remember to compile with -O3)

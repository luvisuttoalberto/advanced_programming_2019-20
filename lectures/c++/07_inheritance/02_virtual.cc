...
struct Animal {
  ...
  //NB: with template it worked also before; the type is known at compile time so the compiler can call the right function (in that case the function of the child)
  ...
  // also, the compiler can't inline virtual functions: he can't know which one is the one to inline!! the parent one or one of the children one?????
...
struct Snake : public Animal {
  ...
  void speak() const noexcept override { std::cout << "ssss\n"; } //c++11 necessary to use override; before we had to use virtual, with the possibility of mistakenly create a different function
};
